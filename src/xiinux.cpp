...
//inline static void strlwr(char*p){while(*p)*p=(char)tolower(*p++);} //?? crashes
//static void strlwr(char*p){while(*p)*p=(char)tolower(*p++);}
//void strlwr(char*p){while(*p)*p=(char)tolower(*p++);}
inline static void strlwr(char*p){while(*p){*p=(char)tolower(*p);p++;}}
...
//							snprintf(buf,sizeof buf,"upload/%s",pth+1);
							snprintf(buf,sizeof buf,"%s",pth+1); //?? mkdir -p user home/uploads
							printf("  upload to [%s]\n",buf);
...
//								printf(" upload in buffer\n");
			...
			case header_value:
				if(c=='\n'){
					*(bufp-1)=0;
					hdrk=strtrm(hdrk,hdrv-2);//?? *hdrv=0
					strlwr(hdrk);
...
//				return;
//			}else
			if(!str||strcasecmp("keep-alive",str)){//?? strncasecmp
//				printf("connection close\n");
				delete this;
				return;
			}
	...
	void process(){
		...
		if(!*path&&qs){
			...
			if(!session_id){
				...
				const struct tm*tm_info=gmtime(&timer);
...
//				printf(" * content:\n%s\n",content);
		...
		if(range&&*range){
			off_t rs=0;
			if(EOF==sscanf(range,"bytes=%zd",&rs)){
				stats.errors++;
...
//	if(!system("alias"))puts("cannot alias c");
//	if(!system("alias p='echo' && p hellow from xii"))puts("cannot system c");
//	if(!system("p=echo echo $p"))puts("cannot system c");
