  ...
  while (!XNextEvent(dpy, &ev)) {
    ...
    case EnterNotify:
      ...
      fprintf(flog, "enternotify   %p   root=%p   subwin=%p\n",
              (void *)ev.xcrossing.window, (void *)ev.xcrossing.root,
              (void *)ev.xcrossing.subwindow);
      fflush(flog);
    ...
    case KeyPress:
      ...
      case 70: { // f4
        ...
        // #define bin_screenshot_area "scrot -s 'scr--%Y-%m-%d---%H-%M-%S.jpg'
        // -e 'mkdir -p ~/img/&&mv $f ~/img/&&feh ~/img/$f'"
        break;
      }
      case 24: // q
        system("xii-qbin");
