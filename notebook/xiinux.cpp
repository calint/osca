...
//------------------------------------
//------------------------------------
static void file_print(const char*filename){
...
#include <sys/types.h>
#include <dirent.h>
static void p(const char*filename){
	puts(filename);
}
...
//#include<unistd.h>
#include<arpa/inet.h>
void print_ifconfig(int fd,const char*iface){
	struct ifreq ifr;
...
//	qdir("/proc",p);
//	file_print("/proc/net/dev");
//	file_print("/proc/net/protocols");
//	file_print("/proc/net/route");
	...
	printf(" ***  %s up: %d\n",ifr.ifr_name,ioctl(fd,SIOCSIFFLAGS,&ifr));
	print_ifconfig(fd,"lo");
	file_print("/proc/net/tcp");
	close(fd);
}
//------------------------------------
//------------------------------------
