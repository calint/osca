...
int main(void) {
  ...
  while (1) {
    ...
    for (size_t i = 0; i < (size_t)length;) {
      struct inotify_event *event = (struct inotify_event *)(void *)&buffer[i];
      ...
      if (strncmp(event->name, "sd", 2) ||
          !isdigit(event->name[strlen(event->name) - 1])) {
        continue;
      }
      char mntdir[128];
      snprintf(mntdir, sizeof(mntdir), "%s/mnt/%s", uhomedir, event->name);
      if (event->mask & IN_CREATE) {
        ...
        while (1) {
          ...
          const int res3 =
              mount(devpth, mntdir, ftype, MS_SYNCHRONOUS | MS_NOATIME, 0);
          if (res3) {
            continue;
          }
          printf("mounte %s %s\n", mntdir, ftype);
        ...
        //				if(res2){printf("%s:%d  cannot
        // chown\n",__FILE__,__LINE__);perror(0);}
        //				if(chown(mntdir,uid,-1)){printf("cannot
        // chown
        //'%s'\n",mntdir);exit(2);}
        ...
        if (res2) {
          printf("%s:%d  cannot umount\n", __FILE__, __LINE__);
          perror("");
        }
        const int res1 = rmdir(mntdir);
        if (res1) {
          printf("%s:%d  cannot rmdir\n", __FILE__, __LINE__);
          perror("");
        }
      }
    }
